...
export class Date{
    ...
    baseData(){
        ...
        return {
            ...
            emptyli:'<li>&nbsp;</li>',
            dateFormat:['YYYY-MM','YYYY-MM-DD','YYYY-MM-DD hh:mm','YYYY-MM-DD hh:mm:ss','YYYY','MM','DD','hh:mm','hh:mm:ss'],//Supported date formats
    ...
    createUi(){
        ...
        for(let i=0; i<len; i++){
            ...
            if(i == 0 && index < 6){
                for(let j=_this.config.beginYear; j<=_this.config.endYear; j++){
                    ul += '<li'+(j==date.getFullYear()?' class="active"':'')+'>'+ j +'Year</li>';
                }
            }else if((i == 1 || index == 6) && index < 7){
                for(let k=1; k<=12; k++){
                    ul += '<li'+(k==date.getMonth() + 1?' class="active"':'')+'>'+ (k<10? '0'+k : k) +'Month</li>';
                }
            }else if((i == 2 || index == 7) && index <= 7){
                let day = _this.bissextile(date.getFullYear(),date.getMonth() + 1);
                for(let l=1; l<=day; l++){
                    ul += '<li'+(l==date.getDate()?' class="active"':'')+'>'+ (l<10? '0'+l : l) +'Day</li>';
                }
            }else if(i == 3 || (index > 7 && i == 0)){
                for(let m=0; m<=23; m++){
                    ul += '<li'+(m==date.getHours()?' class="active"':'')+'>'+ (m<10? '0'+m : m) +'Hours</li>';
                }
            }else if(i == 4 || (index > 7 && i == 1)){
                for(let n=0; n<=59; n+=_this.config.minStep){
                    ul += '<li'+(n==date.getMinutes()?' class="active"':'')+'>'+ (n<10? '0'+n : n) +'Minutes</li>';
                }
            }else if(i == 5 || (index > 7 && i == 2)){
                for(let o=0; o<=59; o++){
                    ul += '<li'+(o==date.getSeconds()?' class="active"':'')+'>'+ (o<10? '0'+o : o) +'Seconds</li>';
                }
            }
        ...
        let $html = `<div class="rolldate-mask"></div>
            <div class="rolldate-wrapper">
                ...
                <footer>
                    <button class="rolldate-btn rolldate-cancel">Cancel</button>
                    <button class="rolldate-btn rolldate-confirm">Set</button>
                </footer>
            </div>`,
